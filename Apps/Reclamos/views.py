from Apps.Reclamos.models import (Clasificacion, Reclamos, Servicio, Tipo_documento,
                                  Estado, Resultado, Etapa, MedioRecepcion, Medidas, MotConcAnt, Tramas)
from .serializers import (ClasificacionSerializer, ReclamosSerializer, ReclamosSerializerData, MedioRecepcionSerializer, ReclamosSerializerList,
                          ...
                          TramaSerializer, ReclamoTramaSerializer)
...
class ReclamosList(generics.ListCreateAPIView):
    ...
    search_fields = ['nombres_p', 'paterno_p', 'materno_p', 'nro_documento_p',
                     'nombres', 'paterno', 'materno', 'nro_documento', 'periodo', 'fecha']
        ...
        if Tramas.objects.filter(periodo=request.data['periodo']).count() > 0:
            ...
            if serializer.is_valid():
                ...
                if request.data["resultado"] in [1, 2]:
                    ...
                    serializerMedidas = MedidasSerializer(
                        data=ListReturn, many=True)
...
def TRAMAS_GP(request, _periodo):
    if request.method == 'GET':
        ...
        if objeto:
            ...
            serializer = ReclamoTramaSerializer(objeto, many=True)
            return Response(serializer.data)
        ...
        if Tramas.objects.all().filter(periodo=periodo_ant.strftime('%Y%m')):
            ...
            if serializer.is_valid():
                serializer.save()
                sig_periodo = datetime.strptime(
                    periodo, '%Y%m') + relativedelta(months=1)
                Reclamos.objects.filter(Q(periodo=periodo) & Q(estado__in=[0, 2, 3])).update(
                    periodo=sig_periodo.strftime('%Y%m'))
                return Response({'status': True, 'message': 'Registro grabado correctamente.'}, status=status.HTTP_201_CREATED)
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        else:
